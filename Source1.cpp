...
bool test1(int x) { //test for the degree = 0
	...
	cout << endl << "The result of the programm for degree n = 0 is ---> " << test.calc(0, test.getcoeff(), x);
	for (int n = 0; ; )
		if (test.calc(n, test.getcoeff(), x) == expected) 
			return true;
		else 
			return false;
}
...
bool test2(int x) { //test for the degree = - 1
	...
	cout << endl << "The result of the programm for degree n = 0 is ---> " << test.calc(-1, test.getcoeff(), x);
	for (int n = - 1; ; )
		if (test.calc(n, test.getcoeff(), x) == expected) //if the degree of the polynomial is negative, the program will not enter the array of its filling and the polynomial will take the value set initially, i.e. 0
			return true;
		else
			return false;
}
...
bool test3(int x) { ////test for the degree = 4
	...
	cout << endl << "The result of the programm for degree n = 4 is ---> " << test.calc(4, test.getcoeff(), x);
	for (int n = 4; ; )
		if (test.calc(n, test.getcoeff(), x) == expected)
			return true;
		else
...
int main() {
	...
	cout << endl << "The value of a polynomial of degree 5 of x = 3:  " << first.calc(n, copy.getcoeff(), x);
